...
$.fancybox.defaults.i18n = {
  ru: {
    ...
    PREV: "Назад",
    ERROR:
      "Контент не может быть загружен. <br/> Пожалуйтста попробуйте ещё раз.",
    DOWNLOAD: "Скачать",
    SHARE: "Поделиться",
    ZOOM: "Увеличить",
  },
...
var OWL_DEFAULT = {
  ...
  nav: false,
  navText: [
    '<span class="owl-icon-wrap"><svg class="owl-icon owl-prev-icon icon"><use xlink:href="/design/sprite.svg#arrow"></use></svg></span>',
    '<span class="owl-icon-wrap"><svg class="owl-icon owl-next-icon icon"><use xlink:href="/design/sprite.svg#arrow"></use></svg></span>',
  ],
  mouseDrag: true,
  touchDrag: true,
  pullDrag: true,
          ...
          success: function (data) {
            ...
            if ("ok" == data.status) {
              ...
              if (typeof data.compare_goods_count != "undefined") {
                ...
                var sidecount = $(
                  ".compare-header .compare-header__counter .num"
                );
          ...
          success: function (data) {
            ...
            if ("ok" == data.status) {
              ...
              if (typeof data.favorites_goods_count != "undefined") {
                // Блок информации о том, что есть товары на сравнении
                var sidecount = $(
                  ".favorites-header .favorites-header__counter .num"
                );
                // Если на сравнении больше нет товаров
                // Указываем информацию о новом количестве товаров на сравнении
                // Блок обновления списка сравнения в каталога
...
function comparePage() {
  ...
  $(".CompareGoodsTableTbodyComparisonLine td:not(.cell)").on(
    "click",
    function () {
...
function mainFunctions() {
  ...
  $(
    "#order-stage-form, .feedback-form, .subscribe-footer__form, .clientForm, .form"
  ).validate();
  ...
  $(
    ".subscribe .subscribe__form, .callback-form, .offer-form, .subscribe-footer .subscribe-footer__form"
    ...
    if ($form.valid()) {
      $.getJSON(url, formData, function (d) {
        var notyText =
          d.status == "ok" ? d.message : "Произошла ошибка, попробуйте ещё раз";
  ...
  $(".header .header-mobile__button, .catalog-button").on(
    "click",
    function (e) {
      e.preventDefault();
      ...
      if (id) {
        ...
        $('.header-content__button[data-target="' + id + '"]').addClass(
          "_active"
        );
      }
    }
  ...
  $(
    ".header .header-content__button-close, .header-content .header-content__overlay"
  ).on("click", function (e) {
    e.preventDefault();
  ...
  function headerMenu() {
    ...
    for (var i = 1; i < menuCount; i++) {
      currentWidth =
        parseInt(
          Math.ceil(
            $(".header-nav .header-nav__item:nth-child(" + i + ")").width()
          )
        ) + 46;
      ...
      if (nextCheck > menuWidth) {
        var a = i;
        for (a; a < menuCount; a++) {
          $(".header-nav .header-nav__item:nth-child(" + a + ")").addClass(
            "mainnav__replaced"
          );
        }
        $(".mainnav .header-nav__list").append(
          '<li class="header-nav__item mainnav__more dropdown _more-menu"><a class="header-nav__link">Еще...</a></li>'
        );
        $(".mainnav__more").append(
          $("<ul>").addClass("overflowMenu dropdown__body")
        );
        ...
        $(".mainnav .mainnav__more").on("click", function () {
          ...
          $(".overflowMenu").hasClass("active")
            ? $(".overflowMenu").removeClass("active")
            : $(".overflowMenu").addClass("active");
          $(".mainnav .header-nav__list").hasClass("active")
            ? $(".mainnav .header-nav__list").removeClass("active")
            : $(".mainnav .header-nav__list").addClass("active");
        });
        $(function ($) {
...
function catalogFunctions() {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        ...
        $(".products-container").prepend(
          '<span class="lds-ellipsis pink"><span></span><span></span><span></span><span></span></span>'
        );
      },
      success: function (d) {
        $(".products-ajax")
  ...
  $(
    ".favorites-goods .products-grid, .compare-goods .products-grid"
  ).owlCarousel(
    $.extend(OWL_DEFAULT, {
      items: 6,
      margin: 9,
  ...
  $(
    ...
    .on("submit", function () {
      ...
      $.ajax({
        ...
        data: formData,
        success: function (data) {
          var $btn = $(".button._add-cart._loading").removeClass(
            "_loading _animated"
          );
          ...
          if (type === "success") {
            ...
            $(".header .cart-header .cart-header__counter .num").html(
              $(data).filter("#newCartCount").html()
            );
            $(".header .cart-header .cart-header__cart-sum").html(
              $(data).filter("#newCartSum").html()
            );
          } else {
            new Noty({
              timeout: "3000",
...
function orderScripts() {
  ...
  $(".quickform select.quickform__select-convenient._period").on(
    "change",
    function () {
  ...
  $(function () {
    $("#quickform .button, #confirmOrder .button")
      .off("click")
      .on("click", function () {
...
(function () {
  ...
  function init(ORDER_DELIVERY, DELIVERY_TYPE) {
    ...
    if (DELIVERY_TYPE === "radio") {
      function hideDeliveryBlock(name) {
        ...
        if (
          ...
          deliveryName.indexOf("Самовывоз") != -1
        ) {
      ...
      $(".delivery-radio").on("change", function () {
        ...
        if (CURRENT_DELIVERY.zoneListEmpty) {
          $(this)
            .closest(".quickform-delivery__item")
            .siblings()
            .find(".delivery-zone-radio")
            .prop("checked", false);
        } else {
          $(this)
            .closest(".quickform-delivery__item")
            .find(".delivery-zone-radio:first")
            .prop("checked", true)
            .trigger("change");
        }
      ...
      $(".delivery-zone-radio").on("change", function () {
        ...
        $(this)
          .closest(".quickform-delivery__item")
          .find(".quickform-delivery__price-default .num")
          .text(addSpaces(getCurrentDeliveryPrice()));
        $(this)
          .closest(".quickform-delivery__item")
          .find(".quickform-delivery__price-zone .num")
          .text(addSpaces(getCurrentDeliveryPrice()));
        changePaymentRadio(deliveryId);
        changeCartSum();
      });
    ...
    function changePaymentSelect(paymentId) {
      ...
      var currentPayment = $.grep(
        CURRENT_DELIVERY.availablePaymentList,
        function (item, i) {
    ...
    function changePaymentRadio(deliveryId) {
      if (deliveryId) {
        var $currentPaymentBlock = $(".quickform-payment").filter(
          '[data-delivery-id="' + deliveryId + '"]'
        );

        $currentPaymentBlock.show().siblings(".quickform-payment").hide();
        $currentPaymentBlock
          .find('[name="form[payment][id]"]:first')
          .prop("checked", true);
      } else {
        $(".quickform-payment")
          .hide()
          .find('[name="form[payment][id]"]')
          .prop("checked", false);
      }
    }
  }
  ...
  function changeCartSum() {
    var currentPriceWithoutChange = Number(
      $(".cart__sum .total-sum").data("total-sum")
    );
    var deliveryPrice = Number(getCurrentDeliveryPrice());
    var discountPrice = Number(
      $(".cart__sum .cart__sum--discount .cart__sum-text-price").data(
        "discount-value"
      )
    );
    // console.log(currentPriceWithoutChange,deliveryPrice,discountPrice)
    // Заполняем стоимость доставки
    $(".cart__sum .cart__sum--delivery-sum .num").text(
      addSpaces(deliveryPrice)
    );
    ...
    $(".cart__sum .total-sum")
      .find(".num")
      .text(addSpaces(Math.ceil(priceNow)));
  }
  ...
  function getCurrentDeliveryPrice() {
    function getPriceFromArray(array) {
      ...
      $.each(array, function (i, el) {
        ...
        if (
          Number(cartSumNow) >= Number(el.sumMorePrice) &&
          Number(maxMorePrice) <= Number(el.sumMorePrice)
        ) {
    ...
    if (CURRENT_DELIVERY.zoneListEmpty) {
      ...
      if (CURRENT_DELIVERY.rulesListEmpty) {
        ...
        var deliveryPrice =
          priceFromArray !== null
            ? priceFromArray
            : Math.ceil(CURRENT_DELIVERY.price);
      ...
      if (zoneList.zoneRuleListEmpty) {
        ...
        var deliveryPrice =
          priceFromArray !== null ? priceFromArray : zoneList.price;
      ...
      .on(
        ...
        $.debounce(300, function () {
          ...
          $.ajax({
            ...
            success: function (data) {
              var $data = $(data);
              var newQtyInputVal = $data
                .find('[data-id="' + id + '"] .qty__input')
                .val();
              ...
              var newPrice = $data
                .find('[data-id="' + id + '"]')
                .find(".cart__product-ajax-price")
                .html();
              $("#cart-ajax-form")
                .find(".cart-ajax__product-price")
                .html(newPrice);
              /**/
        ...
        $.debounce(300, function () {
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              var newItemPrice = $data
                .find('[data-id="' + id + '"] .cart__product-ajax-price')
                .html();
              ...
              $(".cart-header .cart-header__cart-sum .num").text(
                addSpaces(cartSum)
              );

              if ($("#quick_form_coupon_code").val()) {
                $("#quick_form_coupon_code, .coupon-hidden-input").trigger(
                  "change"
                );
              }
              ...
              var $deliveryPriceBlock = $(
                ".cart__sum-row.cart__sum--delivery-sum"
              );
              var deliveryPrice = Number(
                $deliveryPriceBlock.find(".num").text()
              );
    ...
    $.ajax({
      ...
      beforeSend: function () {
        ...
        $submitBtn
          .addClass("_loading")
          .append(
            '<span class="coupon__btn-icon"><i class="fal fa-circle-notch fa-spin"></i></span>'
          );
        $cuponInput.attr("placeholder", "");
      },
      success: function (data) {
        var $discountBlock = $(data)
          .closest("#order-stage-form")
          .find("tr.discount");
        ...
        $(".cart__sum-row.cart__sum--discount .cart__sum-text-price").html(
          discountPercent
        );
        $(".cart__sum-row.cart__sum--discount .discount-type").html(
        ...
        var discountRub = String(
          Math.floor(orderSumDefault) - Math.floor(discount)
        );

        $(".cart__sum-row.cart__sum--discount .cart__sum-text-price").data(
          "discount-value",
          discountRub
        );
          ...
          if (discountType === "coupon") {
            ...
            if (!isResetBtn) {
              $submitBtn
                .addClass("_not-add")
                .find(".coupon__btn-icon")
                .html('<i class="fal fa-times-circle"></i>');
...
$.fn.getColumnContent = function () {
  var $block =
    $(this).length && $(this).hasClass("product-wrap")
      ? $(this).filter(".product-wrap")
      : $("div.product-wrap:eq(0)");
...
function quantity() {
  $(".qty__btn.qty__btn--plus")
    .off("click")
    .on("click", function () {
      var quantity = $(this).parent().find(".qty__input"),
...
function calcDiscount() {
  ...
  if (discountObj) {
    $discountBlock
      .find(".discount-block__percent-num")
      .text(discountObj.percent);
...
function goodspage() {
  ...
  $(".goods-opinion-form__captcha-refresh-button").on("click", function () {
    ...
    $(".goods-opinion-form__captcha-img").attr(
      "src",
      $(".goods-opinion-form__captcha-img").attr("src") +
        "&rand" +
        Math.random(0, 10000)
    );
    return false;
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if ($noticeBlock.data("opinion-type") === "success") {
            ...
            $(".product-view__rating").html(
              $(data).find(".product-view__rating").html()
            );
            $(".goods-opinion-counter").html(
              $(data).find(".goods-opinion-counter").html()
            );
            $(".goods-opinion-list").html(
              $(data).find(".goods-opinion-list").html()
            );
            var opinionsCount = $(".goods-opinion-list").find(
              ".opinion-item"
            ).length;
  ...
  $(".goods-delivery .goods-delivery__title").on("click", function () {
    ...
    if ($btn.hasClass("_active")) {
      $btn
        .html(
          '<svg class="icon"><use xlink:href="/design/sprite.svg#plus-icon"></use></svg>'
        )
        .removeClass("_active");
      return;
    }
    ...
    $btn
      .addClass("_active")
      .html(
        '<svg class="icon"><use xlink:href="/design/sprite.svg#minus-icon"></use></svg>'
      );
  });

  // Сопутствующие | С этим товаром смотрят
  $(
    ".pdt-related-views .products-grid,  .pdt-related .products-grid"
  ).owlCarousel(
    $.extend(OWL_DEFAULT, {
      items: 4,
      responsive: {
...
function goodsMods(inFancybox) {
  ...
  var // Запоминаем поля выбора свойств, для ускорения работы со значениями свойств
    goodsDataProperties = inFancybox
      ? $(
          '.fancybox-content form.product-view__form select[name="form[properties][]"]'
        )
      : $('form.product-view__form select[name="form[properties][]"]'),
    // Запоминаем блоки с информацией по модификациям, для ускорения работы
    goodsDataModifications = inFancybox
      ? $(".fancybox-content div.product-modifications__list")
      : $("div.product-modifications__list");
  ...
  goodsDataProperties.each(function () {
    $(this).on("change", function () {
      var slug =
          getSlugFromGoodsDataFormModificationsProperties(goodsDataProperties),
        modificationBlock = $(
          '.product-modifications__list[rel="' + slug + '"]'
        ),
        ...
        goodsAvailableTrue = goodsAvailable
          .find(".product-view__available-item")
          .filter("._available"),
        goodsAvailableFalse = goodsAvailable
          .find(".product-view__available-item")
          .filter("._not-available"),
        goodsAvailableAddCart = $(
          ".product-view__actions._add-to-form .product-view__add-buttons"
        ),
      ...
      if (modificationBlock.length) {
        ...
        function changePrimaryGoodsImage(goods_mod_image_id) {
          ...
          var // Блок с изображением выбранной модификации товара
            goodsModImageBlock = $(
              '.image-list [data-id="' + parseInt(goods_mod_image_id) + '"'
            ),
...
function initTabs() {
  ...
  if (!isFind) {
    ...
    var tab = $(".tabs-list .tabs-list__item .tabs-list__item-link")
      .attr("id")
      .slice(-1);
    tabSwitch(tab, true);
  }
        ...
        for (var i = 0; i <= items; i++) {
          var currentColor =
            i >= colorsArray.length
              ? colorsArray[Math.abs(colorsArray.length - i)]
              : colorsArray[i];

          $(element)
            .find(".owl-item")
            .eq(i)
            .find(".categories-list__item")
      ...
      .each(function (i, el) {
        ...
        $bannersMobile.find(".banner").each(function () {
          ...
          $bannerLink.attr(
            ...
            $bannerLink.attr("data-mobile-bg")
          );
        });
        lozad().observe();
      });
...
function SearchFieldInit(obj) {
  ...
  if (typeof obj.SearchFieldCheck != "function") {
    console.log(
      "function SearchFieldCheck is not found in object for SearchFieldInit",
      { status: "error" }
    );
    return false;
    // Проверка, сколько полей поиска нам подсунули за раз на инициализацию
  } else if (1 < obj.f_search.length) {
    console.log("function SearchFieldInit must have only one search object", {
      status: "error",
    });
    return false;
  }
  ...
  document.SearchInCatalogAjaxQuerySender[randHash] = function (old_val) {
    ...
    if (typeof sessionStorage !== "undefined") {
      try {
        if (sessionStorage.getItem("lastSearchQueryArray")) {
          last_search_query_array = JSON.parse(
            sessionStorage.getItem("lastSearchQueryArray")
          );
        ...
        success: function (data) {
          ...
          if (typeof sessionStorage !== "undefined") {
            try {
              ...
              // Находим соответствие текущего запроса с sessionStorage
              var currentSearch = $.grep(
                last_search_query_array,
                function (item) {
                  return item.search_query == old_val;
                }
              )[0];
    ...
    function showDropdownSearch(data) {
      ...
      if (data.category.length != undefined && data.category.length > 0) {
        ...
        $("#search-result").removeClass("_active").hide();
        $("#search-result .search__result-inner .result-category")
          .find(".result__title")
          .remove();
        $("#search-result .search__result-inner .result-category").append(
          '<h3 class="result__title">Категории</h3>'
        );
      ...
      if (data.goods.length != undefined && data.goods.length > 0) {
        ...
        $("#search-result .search__result-inner .result-goods")
          .find(".result__title")
          .remove();
        $("#search-result .search__result-inner .result-goods").append(
          '<h3 class="result__title">Товары</h3>'
        );
